...
MCP tool handlers exposed to clients over the official Go MCP SDK's stdio
transport. One handler per user-visible operation; validation, encryption
annotation, and output shaping live here.
...
#### Scenario: refusing an oversized download
- **WHEN** the caller invokes `download_resource` for a resource whose metadata
  reports a size larger than `MaxResourceBytes`
- **THEN** the server returns an error mentioning the size limit and does NOT
  download the bytes

#### Scenario: refusing an oversized upload
- **WHEN** the caller invokes `upload_resource` with base64 data whose decoded
  size exceeds `MaxResourceBytes`
- **THEN** the server returns an error mentioning the size limit and does NOT
  call Joplin
